...
class PhpBuf_Message_Abstract {
	...
	public function read(PhpBuf_IO_Reader_Interface $reader) {
		...
		} catch (PhpBuf_IO_Exception $e) { return; }
	}
	/**
	...
	protected function setField($name, $type, $rule, $index, $extra = "") {
		if($type == PhpBuf_Type::MESSAGE && (!is_string($extra) || empty($extra))) { throw new PhpBuf_Message_Exception("message mast have \$extra");}
		if($type == PhpBuf_Type::ENUM && (!is_array($extra) || empty($extra))) { throw new PhpBuf_Message_Exception("enum mast have \$extra");}
	...
	protected function getValue($field, $throwException = true) {
		...
		if($throwException) { throw new PhpBuf_Message_Exception("property $field not found"); }
	}
	/**
	...
	protected function readFieldFromHeader(PhpBuf_IO_Reader_Interface $reader) {
		...
		$fieldIndex = $varint >> 3;
		if(!isset($this->fields[$fieldIndex])) { throw new PhpBuf_Field_Exception("index not found"); }
		$fieldClass = $this->fields[$fieldIndex];
		$fieldsWireType = $fieldClass->getWireType();
		if($wireType != $fieldsWireType) { throw new PhpBuf_Field_Exception("discrepancy of wire types"); }
		return $fieldClass;
	}
}
?>